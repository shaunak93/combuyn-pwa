...
const GenericSelector = (props) => {
    let {labelFontSize, backgroundColor, header ,options, selectedOption, onChange} = props;

    return (
        <div style={{position: 'relative', minHeight: '28px',  margin: '10px 0',boxSizing:'border-box',
        borderRadius: '8px', backgroundColor: backgroundColor || 'transparent'}}>
        <div style={{display: 'inline-block', width: '70px', height: '28px', verticalAlign: 'top'}}>
            <p style={{fontSize: labelFontSize || '16px', fontWeight: 'bold', color: '#4D4D4D', margin: '0', width: 'max-content', paddingTop: '10px'}}>{header}</p>
        </div>
...
const GenericOptions = (props) => {
    let {onClick, value, isSelected} = props;
