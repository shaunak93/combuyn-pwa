...
const getCampaignStatus = (is_active, start_datetime, end_datetime, currentTime) => {
...
function ProductCampaignCard(props) {
    ...
    let {
        ...
        smallDescription, original_price, selling_price,
        total_order_count, current_order_count
    } = campaignDetails;
    let status = getCampaignStatus(is_active, start_datetime, end_datetime, currentTime);

    const shareCampaign = (e) => {
        e.preventDefault();
        e.stopPropagation();
        fetch("https://images.unsplash.com/photo-1542291026-7eec264c27ff?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=2850&q=80")
    ...
    return (
        <div className="product-campaign-card" onClick={()=>{onClick(campaignDetails.id)}} style={{position: 'relative', width: `${isFullWidth ? '100%': '232px' }`, height: '226', borderRadius: '4px', backgroundColor:'#E6E6E6', marginRight: '15px'}}>
            <div style={{position: 'relative', width: 'inherit', position: 'relative', height:'136px', borderRadius: '4px', overflow: 'hidden'}}>
                <img style={{width: 'inherit', height:"inherit", borderRadius: '4px'}} 
                    src={image} 
                    alt="Loading..."
                />
                ...
                <div style={{position: 'absolute', right: '6px', bottom: '10px', width: '75px', height:"30px"}}>
                    <Button onClick={shareCampaign} style={{height: 'inherit', width: 'inherit', border: '2px solid #ffffff', borderRadius: '15px', fontSize: '12px', fontWeight: 'bold', textTransform: 'capitalize', backgroundColor:'#FFA64D', color: '#ffffff'}} 
            ...
            <div style={{width: 'inherit', height:'60px', borderRadius: '4px', backgroundColor:'#E6E6E6'}}>
                <Grid
                  container
                  spacing={0}
                ...
                >
                    <Grid item  xs={8} style={{padding: '10px 12px 0'}}>
                        <p style={{margin: '0', fontSize: '15px', fontWeight: 'bold'}}>{name}</p>
                        <p style={{margin: '0', fontSize: '12px'}}>{smallDescription || ''}</p>
                    </Grid>
                    <Grid item  xs={4} style={{padding: '10px 12px 0'}}>
                        <p style={{margin: '0', fontSize: '15px', fontWeight: 'bold', textAlign: 'right'}}> &#8377;{selling_price || "-"}</p>
                    ...
                    {status === 'upcoming' && 
                        <Grid item  xs={12} style={{padding: '4px 12px 0'}}>
                            <p style={{margin: '0', fontSize: '10px', textAlign: 'center'}}>
                                ...
                                <span style={{ width: 'max-content',color: '#3785B8', fontWeight: 'bold', margin: '0'}}>&nbsp; {total_order_count} Pcs</span>
                            </p>
                        </Grid>
                    }
            ...
            {status === 'active' && 
                <>
                    <div style={{position: 'absolute',left: '7px', top: '187px',  width: '214px', height:'10px', borderRadius: '7px', backgroundColor:'#ffffff', padding: '2px'}}>
                        <div style={{borderRadius: '5px',  width: `${getFulfilledPecentage(current_order_count, total_order_count)}%`, height:'10px', backgroundColor:'#48C28B', minWidth: 'min-content'}}>
                            <p style={{color: '#ffffff', fontSize: '8px', width: 'fit-content', margin: '0', padding: '0 4px', fontWeight:'bold'}}>{`${getFulfilledPecentage(current_order_count, total_order_count)}%`}</p>
                        </div>
                    </div>
                    <div style={{width: 'inherit', height:'30px', borderRadius: '4px'}}>
                        <span style={{width: '100%', textAlign: 'center', display: 'block', paddingTop: '8px'}}>
                            ...
                            <span style={{fontSize: '12px', margin: '0', color: '#3785B8', fontWeight: '500'}}>{getCountdownTimeString(end_datetime, currentTime)}</span>
                        </span>
                    </div>
                </>
            ...
            {status === 'upcoming' && 
                <div style={{width: 'inherit', height:'30px', borderRadius: '4px'}}>
                    <span style={{width: '100%', textAlign: 'center', display: 'block', paddingTop: '8px'}}>
                        ...
                        <span style={{fontSize: '12px', margin: '0', color: '#3785B8', fontWeight: '500'}}>{getCountdownTimeString(currentTime, start_datetime)}</span>
                    </span>
                </div>
            }

            {status === 'finished' && 
                <div style={{width: 'inherit', height:'30px', backgroundColor: '#E6E6E6', textAlign: 'center'}}>
